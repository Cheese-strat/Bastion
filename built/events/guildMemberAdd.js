...
module.exports = function (client, member) {
    ...
    if (member.guild.id === "609331117920747521") {
        if (!member.bot) {
            var vrole = member.guild.roles.find(function (role) { return role.name === "OP beginnings"; });
            ...
            var vrole = member.guild.roles.find(function (role) { return role.name === "Bots"; });
            member.addRole(vrole);
            member.guild.channels
                .get("642375063252893696")
    ...
    if (member.guild.id === "491564959835226112") {
        if (!member.bot) {
            var vrole = member.guild.roles.find(function (role) { return role.name === "newbie"; });
            member.addRole(vrole);
            member.guild.channels
