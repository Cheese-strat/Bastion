...
module.exports = {
    ...
    execute: function (msg, args, pokemon) {
        ...
        if (startlvl < 4) {
            var endcost = Math.floor(speed * Math.pow(endlvl, (1.5 + speed - 1)));
            return msg.channel.send("Your final result is: " + endcost + "c");
        }
        else {
            var endcost = speed * Math.pow(endlvl, (1.5 + speed - 1));
            var startcost = speed * Math.pow(startlvl, (1.5 + speed - 1));
