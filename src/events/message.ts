...
export default class messageEVENT extends Event<"message"> {
  ...
  async execute(client: clientClass, msg: MessageTYPE) {
    ...
    let args: string[] = msg.content
      .slice(prefix.length)
      .trim()
      .split(/ +/);
    const commandName = args.shift();

    if (commandName === `crash` && client.developers.includes(msg.author.id))
      throw new Error(`Crashing on authorization of ${msg.author.tag}`);
    if (!commandName) return false;
    console.log(client.commands) //ERROR problem with the handler, this logs an empty collection
    const command =
...
//console.log(command)
    ...
    if (command.args && !args.length) {
      ...
      if (command.args.usage) {
        reply += `\nThe correct usage is: \`${prefix}${command.name} ${command.args.usage}\``;
      }
